 ...
 */
public class GetRoomByIdTask extends AsyncTask<String, Void, Room> {
    @Override
    ...
    private Room deserializeCoordinate(String urlString) throws IOException {
    ...
    protected String getURL(String id) {
        return Constants.CONNECTION_PROTOCOL + Constants.COLON_AND_TWO_SLASHES + Constants.IP + Constants.COLON +
                Constants.PORT + Constants.SLASH_RESOURCES_SLASH + Constants.ROOM + Constants.QUESTION_MARK_ID_EQUALS + id;
    }
}
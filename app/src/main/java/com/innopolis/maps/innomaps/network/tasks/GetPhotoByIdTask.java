 ...
 */
public class GetPhotoByIdTask extends AsyncTask<String, Void, Photo> {
    @Override
    ...
    private Photo deserializeCoordinate(String urlString) throws IOException {
        ObjectMapper mapper = new ObjectMapper();
        String response;
        try {
            response = NetworkController.establishGetConnection(urlString);
            mapper.setDateFormat(Constants.serverDateFormat);
            return mapper.readValue(response, Photo.class);
    ...
    protected String getURL(String id) {
        return Constants.CONNECTION_PROTOCOL + Constants.COLON_AND_TWO_SLASHES + Constants.IP + Constants.COLON +
                Constants.PORT + Constants.SLASH_RESOURCES_SLASH + Constants.PHOTO + Constants.QUESTION_MARK_ID_EQUALS + id;
    }
}
 ...
 */
public class GetEventScheduleByIdTask extends AsyncTask<String, Void, EventSchedule> {
    @Override
    protected EventSchedule doInBackground(String... params) {
        try {
    ...
    private EventSchedule deserializeCoordinate(String urlString) throws IOException {
        ObjectMapper mapper = new ObjectMapper();
        String response;
        try {
            response = NetworkController.establishGetConnection(urlString);
            mapper.setDateFormat(Constants.serverDateFormat);
            return mapper.readValue(response, EventSchedule.class);
    ...
    protected String getURL(String id) {
        return Constants.CONNECTION_PROTOCOL + Constants.COLON_AND_TWO_SLASHES + Constants.IP + Constants.COLON + Constants.PORT +
                Constants.SLASH_RESOURCES_SLASH + Constants.EVENT + Constants.SCHEDULE + Constants.QUESTION_MARK_ID_EQUALS + id;
    }
}
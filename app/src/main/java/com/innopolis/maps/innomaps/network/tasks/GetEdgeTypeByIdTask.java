 ...
 */
public class GetEdgeTypeByIdTask extends AsyncTask<String, Void, EdgeType> {
    @Override
    protected EdgeType doInBackground(String... params) {
        try {
    ...
    private EdgeType deserializeCoordinate(String urlString) throws IOException {
        ObjectMapper mapper = new ObjectMapper();
        String response;
        try {
            response = NetworkController.establishGetConnection(urlString);
            mapper.setDateFormat(Constants.serverDateFormat);
            return mapper.readValue(response, EdgeType.class);
